...
<?php if(adminloggedin()){
	...
	<body class="fade-in one" style="background-color:#00000"> 
		...
		$str="<a href=\"https://docs.google.com/gview?url=http://masterji.dhruvsrivastav.com/Notes/".$branch."/".$sem."/".$subwospace."/".$id2."/".$deltext."\">".$del."</a><br>";
					...
					$query4=mysqli_query($con,$sql4)  or header("Location: http://masterjimob.dhruvsrivastav.com/404.html");
					unlink($target_dir."/".$deltext) or header("Location: http://masterjimob.dhruvsrivastav.com/404.html");
				}
				
				$sql="SELECT * FROM ".$subwospace." order by `id`;";
				$result = mysqli_query($con,$sql) or header("Location: http://masterjimob.dhruvsrivastav.com/404.html");
